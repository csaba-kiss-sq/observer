...
class Observer_data_m extends MY_Model
{
	protected $_table = 'observer_data';

	public function get_by($key = nullf, $value = null)
	{
		$this->db
			->select('observer_data.*')
			->join('observer_merchants', 'observer_merchants.id = observer_data.observer_merchants_id', 'left')
			->join('observer_products', 'observer_products.id = observer_data.observer_products_id', 'left');
